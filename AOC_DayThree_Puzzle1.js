...
function wireCordPath(stringArray){
  ...
  for(var i=0; i< stringArray.length; i++){
     instruction = stringArray[i];
     direction = instruction.substring(0,1);
     //console.log("Direction = " + direction);
     distance = instruction.substring(1,instruction.length);
     //console.log("Distance = " + distance);
          //Add coordinates into multi array
     if(direction == "R"){
     		// x is going to change positive,  y stays the same
     		//console.log("Yup, you are going right...");
        for(counter = 0; counter < distance; counter++){
      	...
      	//console.log("Yup, you are going UP...");
        // x stays same,  but y going to change positive...
        for(counter = 0; counter < distance; counter++){
        	y = y + 1;
          //console.log("(" + x + "," + y + ")");
          newArray.push( x + "," + y );
        ...
        // x is going in the negative direction, y stays same...
         	//console.log("Yup, you are going left...");
          for(counter = 0; counter < distance; counter++){
            x = x - 1;
          	//console.log("(" + x + "," + y + ")");
            newArray.push( x + "," + y  );
          	...
          	//console.log("Yup, you are going down...");
           // x stays same,  but y going to change negative...
            for(counter = 0; counter < distance; counter++){
              y = y - 1;
         			//console.log("(" + x + "," + y + ")");
              newArray.push( x + "," + y );
            	...
            	console.log("WOAH....  Something went wrong Ava");
  }
...
wireOneCoords = wireCordPath(testOne);
wireTwoCoords = wireCordPath(testTwo);
console.log(wireOneCoords);
console.log(wireTwoCoords);

var arrayOneLength = wireOneCoords.length;
var arrayTwoLength = wireTwoCoords.length;
var manhattanDistance = 35000;
var testing = 0;
var lastArray = new Array();


for(var h = 0; h < arrayOneLength; h++){
	for(var d = 0; d < arrayTwoLength; d++){
  
  	//console.log("Does equal ??" + wireOneCoords[h] + "," +  wireTwoCoords[d] );
  	if(wireOneCoords[h] == wireTwoCoords[d]){
    	//console.log(wireOneCoords[h]);
      lastArray = wireOneCoords[h].split(',');
      //console.log(lastArray[0] + " and " + lastArray[1]);
      testing = Math.abs(Number(lastArray[0])) + Math.abs(Number(lastArray[1]));
      if(testing < manhattanDistance){
      	manhattanDistance = testing;
        console.log("current manhattanDistance = " + manhattanDistance );
      }
